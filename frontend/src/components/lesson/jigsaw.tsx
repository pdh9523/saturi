...
export default function Icon({ dataGroup, progressData, onJigsawClick }: JigsawProps) {
  ...
  // Popover 부분  
  const [anchorEl, setAnchorEl] = useState<HTMLElement | SVGPathElement | null>(null);          // SVGPathElemet 부분도 추가함.
  ...
  const startLesson = () => {
    router.push(`${currentPath}/${dataGroup[selectedIndex.current].lessonGroupId}`);
  };
